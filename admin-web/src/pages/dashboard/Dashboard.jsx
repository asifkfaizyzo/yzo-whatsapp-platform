...
export default function Dashboard() {
  ...
  const pendingTenantsCount = tenants.filter(
    (t) => t.status === "PENDING"
  ).length;
  ...
  // ── Recent Activities ──
  const recentActivities = tenants.slice(0, 5).map((tenant) => {
    ...
    return { id: tenant.id, text: actionText, time: timeText, color: colorClass };
  });
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Monitor platform system load, control tenant nodes, and handle
        ...
        <button
          ...
          className="btn-secondary flex items-center justify-center gap-2 px-4 py-2.5
                     rounded-xl shadow-sm hover:shadow transition duration-150
        ...
        <div className="card p-6 flex items-start justify-between">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-1 text-xs text-gray-500 font-medium">
              <span>
                {totalTenantsCount
                  ? Math.round((activeTenantsCount / totalTenantsCount) * 100)
                  : 0}% active rate
              </span>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            {revenueLoading ? (
              ...
              <p className="text-3xl font-bold text-gray-900">
                {formatINR(revenueStats?.thisMonthRevenue)}
              </p>
            )}
            {revenueLoading ? (
              ...
              <div
                className={`inline-flex items-center gap-1 text-xs font-semibold
                  ...
                  (revenueStats?.growthPercent ?? 0) >= 0
              ...
              >
                ...
                <span>
                  {(revenueStats?.growthPercent ?? 0) >= 0 ? "+" : ""}
                  {revenueStats?.growthPercent ?? 0}% vs last month
                </span>
              </div>
            )}
          </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="flex items-center justify-between pb-2 border-b border-gray-100">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                Real-time tenant onboarding and instance statuses
              </p>
            </div>
            <Link
              to="/dashboard/tenants"
              className="text-xs font-semibold text-[#125EF2] hover:underline
          ...
          {loading ? (
            ...
            <div className="py-12 text-center">
              ...
              <p className="text-xs text-gray-400 mt-1">
                Tenant accounts will show up here as they register.
              </p>
            </div>
          ) : (
            <div className="divide-y divide-gray-100">
              {recentActivities.map((act) => (
                <div key={act.id} className="py-3.5 flex items-start gap-3">
                  <span
                    className={`h-2.5 w-2.5 mt-1.5 rounded-full ${act.color} flex-shrink-0`}
                  />
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-gray-800">{act.text}</p>
                    <p className="text-xs text-gray-400 mt-0.5">
                      Date: {act.time} • System Onboard
                    </p>
                  </div>
                </div>
              ))}
        ...
        {/* Right Column: Quick Guide + Navigation */}
        <div className="space-y-6">
          <div className="card p-6 space-y-4 bg-gradient-to-br from-[#125EF2]
                          to-[#125EF2] text-white border-transparent shadow-md">
            <h3 className="font-bold text-base flex items-center gap-2">
              <Shield size={18} />
              <span>Super Admin Access</span>
            </h3>
            <p className="text-sm text-[#CFE0FD] leading-relaxed">
              You are logged in with root platform privileges. Use the links
              below or the sidebar to review business analytics, register manual
              nodes, or toggle security configurations.
            </p>
            <div className="bg-[#125EF2]/50 rounded-2xl p-4 border border-[#125EF2]/20">
              <p className="text-xs font-semibold uppercase tracking-wider text-[#CFE0FD]">
                System Gateway API
              </p>
            ...
            <div className="space-y-2">
              <Link
                ...
                className="flex items-center justify-between p-2.5 rounded-xl
                           ...
                           hover:text-[#0F4FCC] hover:bg-[#EAF2FE]/50

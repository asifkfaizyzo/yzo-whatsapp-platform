...
export default function TopNavBar() {
  ...
  useEffect(() => {
    ...
    // ✅ Get stable socket (won't recreate if already exists)
    const socket = getSocket();

    // ✅ Join correct room only once
    const joinRoom = () => {
      if (socketJoined.current) return; // ← prevent duplicate joins
    ...
    // ✅ On connect (handles reconnection too)
    socket.on("connect", () => {
    ...
    if (userType === "TENANT") {
      ...
      // ✅ User raised ticket → Tenant notified
      ...
      // ✅ SuperAdmin replied → Tenant notified
      ...
      // ✅ Escalation confirmation → Tenant notified
      ...
      // ✅ SuperAdmin resolved/closed → Tenant notified
  ...
  }, [authUser?.id, authUser?.type]); // ✅ only re-run if id or type changes

  // ── Disconnect socket on logout (call this on logout) ──
